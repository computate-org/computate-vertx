...
public class UseCaseGenerator extends UseCaseGeneratorGen<FiwareGenerator> {
	...
	protected void _useCaseSummaryJsonldPath(Wrap<String> c) {
		c.o(appPath + "-static/fiware/context.jsonld");
	}
	protected void _useCaseSummaryJsonldFile(Wrap<File> c) {
		c.o(new File(useCaseSummaryJsonldPath));
	}
	protected void _wFiwareContext(Wrap<AllWriter> c) {
		c.o(AllWriter.create(siteRequest_, useCaseSummaryJsonldFile, "  "));
	}
	...
	public Future<Void> writeUseCase() {
		...
		loadSmartDataModels().onSuccess(a -> {
			...
			wFiwareContext.l(new JsonObject().put("@context", useCaseSummary).encodePrettily());
	...
	public Future<Void> loadUseCaseModels() {
		Promise<Void> promise = Promise.promise();
			...
			siteRequest_.getWebClient().get(solrPort, solrHostName, solrRequestUri).ssl(solrSsl).authentication(new UsernamePasswordCredentials(solrUsername, solrPassword)).send().onSuccess(a -> {
				try {
					...
					vertx_.fileSystem().mkdirs(fiwareSwaggerYamlFile.getParent()).onComplete(b -> {
						...
						useCaseSummary.put("ngsi-ld", "https://uri.etsi.org/ngsi-ld/");
						useCaseSummary.put("fiware", "https://uri.fiware.org/ns/data-models#");
						useCaseSummary.put("schema", "https://schema.org/");
						useCaseSummary.put("smartvillage", "https://raw.githubusercontent.com/" + config.getString(ComputateConfigKeys.GITHUB_ORG) + "/" + config.getString(ComputateConfigKeys.SITE_NAME) + "-static/main/fiware/");
...
//			useCaseSummary.put(classDoc.getClassSimpleName(), "https://github.com/" + config.getString(ComputateConfigKeys.GITHUB_ORG) + "/" + config.getString(ComputateConfigKeys.SITE_NAME) + "-static/blob/main/fiware/" + classDoc.getClassSimpleName() + "/");
			useCaseSummary.put(classDoc.getClassSimpleName(), "smartvillage:" + classDoc.getClassSimpleName());
				...
				try {
					...
					for(Long k = searchEntities.getStart(); k < queryResponse.getResponse().getNumFound(); k+=searchEntitesLines) {
						for(Integer l = 0; l < searchEntitiesResults.size(); l++) {
							...
							useCaseSummary.put(entityVar, Optional.ofNullable(entityFiwareContext).orElse("https://github.com/" + config.getString(ComputateConfigKeys.GITHUB_ORG) + "/" + config.getString(ComputateConfigKeys.SITE_NAME) + "-static/blob/main/fiware/" + classDoc.getClassSimpleName() + "/attributes/" + entityVar + ".md"));
