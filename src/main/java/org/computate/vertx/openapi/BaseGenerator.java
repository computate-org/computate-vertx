      ...
      siteRequest_.getWebClient().get(solrPort, solrHostName, solrRequestUri).ssl(solrSsl).authentication(new UsernamePasswordCredentials(solrUsername, solrPassword)).send().onSuccess(a -> {
        try {
          ...
          loadClass(queryResponse.getResponse().getDocs(), 0).onSuccess(b -> {
            ...
            LOG.error(String.format("generating classes failed. "), ex);
            promise.fail(ex);
          }); 
        } catch(Exception ex) {
  ...
  public Future<Void> loadClass(List<SolrResponse.Doc> docs, Integer i) {
    ...
    try {
      if(docs.size() >= (i + 1)) {
        ...
        toutesLangues.stream().map(o -> (String)o).forEach(classLanguage -> {
          futures.add(Future.future(promise1 -> {
            loadLanguageClass(classLanguage, classDoc, doc, apiWriters, classUris).onSuccess(a -> {
              ...
              LOG.error(String.format("generating class failed. "), ex);
              promise1.fail(ex);
            });
          }));
        });
          ...
          try {
            ...
            loadEntities(classDoc, apiWriters).onSuccess(a -> {
              try {
                ...
                loadClass(docs, i + 1).onSuccess(c -> {
                  promise.complete();
                }).onFailure(ex -> {
                  promise.fail(ex);
                });
              ...
              LOG.error("write class writers failed", ex);
              promise.fail(ex);
          }
        }).onFailure(ex -> {
          promise.fail(ex);
        ...
        LOG.error("load class failed", ex);
        promise.fail(ex);
    }
    return promise.future();
  }

  public Future<Void> loadLanguageClass(String classLanguage, ComputateEnUSClass classDoc, SolrResponse.Doc doc, List<ApiWriter> apiWriters, ArrayList<String> classUris) {
    Promise<Void> promise = Promise.promise();
    try {
      List<String> defaultFacets = Optional.ofNullable((List<String>)doc.get("classeFacetsDefaut_stored_strings")).orElse(Arrays.asList());
      List<String> classApiMethods = (List<String>)doc.get(String.format("classeApiMethodes_%s_stored_strings", classLanguage));
      if(classApiMethods != null) {
        ...
        for(String classApiMethod : classApiMethods) {
          ...
          apiWriter.setDefaultLanguageName(languageName);
          apiWriter.setLanguageName(classLanguage);
        ...
        LOG.error("load language class failed", ex);
        promise.fail(ex);
    }
    return promise.future();
  }
