				...
				try {
					...
					for(Long k = searchEntities.getStart(); k < queryResponse.getResponse().getNumFound(); k+=searchEntitesLines) {
						for(Integer l = 0; l < searchEntitiesResults.size(); l++) {
							...
							fiwareContext.put(entityVar, Optional.ofNullable(entityFiwareContext).orElse("https://github.com/" + config.getString(ComputateConfigKeys.GIT_ORG) + "/" + config.getString(ComputateConfigKeys.SITE_NAME) + "/" + classDoc.getClassSimpleName() + "/attributes/" + entityVar + ".md"));
