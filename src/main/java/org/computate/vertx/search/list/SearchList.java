...
import org.computate.search.response.solr.SolrResponse.ResponseHeader;
...
public class SearchList<DEV> extends SearchListGen<DEV> implements Iterable<DEV> {
  ...
  public SearchList<DEV> bq(String s) {
    request.bq(s);
    return this;
  }
  public List<String> getBoostQueries() {
    return request.getBoostQueries();
  }
   ...
   * Ignore: true
   */
  protected void _scope(Wrap<String> w) {
    w.o("GET");
  }
  ...
  public Future<Boolean> next() {
    ...
    try {
      ...
      if(siteRequest_.getPublicRead() || siteRequest_.getScopes().contains(scope)) {
        ...
        ResponseHeader responseHeader = new SolrResponse.ResponseHeader();
        responseHeader.setqTime(0);
